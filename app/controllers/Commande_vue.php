...
class Commande_vue extends Controller
{
    ...
    public function bouton_validateur(int $id_commande, array $lignes_commande): void
    {
        ...
        if ($jwt->est_valide($token) && !$jwt->est_expire($token) && $jwt->verifier_validite($token)) {
            ...
            if ($role == 'validateur') {
                ...
                $lignesCommandeJson = json_encode($lignes_commande, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE);
            ...
            <script>
                function confirmer_commande() {
                    ...
                    data.append("id_commande", ' . $id_commande . ');
                    data.append("id_validateur", ' . $id_valideur . ');
                    data.append("lignes_commande", JSON.stringify(' . $lignesCommandeJson . '));
    ...
    public function accepter_commande(): void
    {
        ...
        if (!isset($_POST['id_commande'], $_POST['id_validateur'], $_POST['lignes_commande'])) {
            // Gérer l'absence de données nécessaires
            error_log('Données nécessaires pour accepter la commande manquantes');
        ...
        if (is_array($lignes_commande)) {
            ...
            foreach ($lignes_commande as $ligne) {
                if (isset($ligne['id_produit'], $ligne['quantite'])) {
                    $produit = new Produit();
                    $produit->diminuer_quantite($ligne['id_produit'], $ligne['quantite']);
                } else {
                    error_log('Données de ligne de commande manquantes ou mal formées');
                }
            }
        } else {
