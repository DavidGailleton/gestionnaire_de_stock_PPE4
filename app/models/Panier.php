...
class Panier extends Model
{
    ...
    public function selectionner_materiels_du_panier(int $id_utilisateur): array
    {
        ...
        $query =
            "SELECT materiels.id_pro AS id, produits.libelle_pro AS libelle, produits.description_pro AS description, produits.qte_stock_pro AS quantite_stock FROM panier INNER JOIN materiels on panier.id_pro = materiels.id_pro INNER JOIN ppe4.produits on panier.id_pro = produits.id_pro WHERE id_uti = :id_utilisateur";
        $stmt = $this->pdo->prepare($query);
        $stmt->bindValue("id_utilisateur", $id_utilisateur, \PDO::PARAM_INT);
        $stmt->execute();
